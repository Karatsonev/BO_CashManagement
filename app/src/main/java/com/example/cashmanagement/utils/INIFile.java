...
public class INIFile
{
    ...
    public void setINIAttributes(String appName, String fileName  )
    {
        String pathName = Environment.getExternalStorageDirectory() + appName + File.separator;

        if( fileName.equals( "" ) ) fileName = appName + ".Ini";

        this.mpropEnv = getEnvVars();
        this.mhmapSections = new LinkedHashMap();
        this.mstrFile = pathName + fileName;
        // Load the specified INI file.
        if( checkFile( this.mstrFile ) )
        {
            load( getFileName() );
        }
    }

    private void setINIFile( String pstrPathAndName )
    {
        this.mpropEnv = getEnvVars();
        this.mhmapSections = new LinkedHashMap();
    ...
    {
        setINIFile( pstrPathAndName );
    }

    /**
     * Create a iniFile object from the current user folder
     */
    public INIFile()//FilenameUtils.getBaseName(
    {
        setINIFile( Environment.getExternalStorageDirectory() +  "MobileClubCard"  + ".Ini" );//
    }
    ...
    public Date getDateProperty(String pstrSection, String pstrProp )
    {
        ...
        if( objSec != null )
        {
            ...
            catch( ParseException | IllegalArgumentException PExIgnore )
            {
            }
            finally
            {
    ...
    /**
     * Sets the ini file name being used.
     *
     * @return nothing
     */
     ...
     * @return
     */
    public boolean save( String iniFile )
    {
        boolean blnRet = false;
        File objFile = null;
        String strName = null;
    ...
    private boolean checkDateTimeFormat( String pstrDtFmt )
    {
        ...
        catch( NullPointerException | IllegalArgumentException NPExIgnore )
        {
        }
        finally
        {
    ...
    public void load( String file )
    {
        ...
        catch( IOException | NullPointerException IOExIgnore )
        {
            this.mhmapSections.clear();
        }
        finally
     ...
     *
     * public static void main(String[] pstrArgs) { INIFile objINI = null;
     * String strFile = null;
     *
     * if (pstrArgs.length == 0) return;
     *
     * strFile = pstrArgs[0]; // Following call will load the strFile if one
     * exists. objINI = new INIFile(strFile);
     *
     * // objINI.addSection("QADatabase", "QA database connection details\nUsed
     * for QA Testing"); // objINI.setStringProperty("QADatabase", "SID",
     * "ORCL", null); // objINI.setStringProperty("QADatabase", "UserId",
     * "System", null); // objINI.setStringProperty("QADatabase", "Password",
     * "Manager", null); // objINI.setStringProperty("QADatabase", "HostName",
     * "DBServer", null); // objINI.setIntegerProperty("QADatabase", "Port",
     * 1521, null); // objINI.setStringProperty("QADatabase", "OracleHome",
     * "%ORACLE_HOME%", null); // //objINI.setSecComments("Folders",
     * "Directories where generated files are stored");
     * objINI.setStringProperty("Folders", "folder1", "G:\\Temp", null);
     * objINI.setStringProperty("Folders", "folder2", "G:\\Temp\\Backup", null);
     *
     * // Save changes back to strFile. objINI.save(); objINI = null; }
     *
     * private INIFile(String pathAndName) { throw new
